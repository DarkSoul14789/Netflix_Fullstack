...
//Get all or get newest 10
router.get("/",verify, async (req,res)=>{
  const query = req.query.new;
  const type = req.query.type;
  if(req.user.isAdmin){
    try {
      const movies = query ? type==="series"||type===undefined ? await Movie.find({isSeries:true}).sort({_id:-1}).limit(10) : await Movie.find({isSeries:false}).sort({_id:-1}).limit(10) : await Movie.find();
